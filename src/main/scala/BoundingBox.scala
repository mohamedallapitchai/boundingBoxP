...
object BoundingBox {
  ...
  def findMaxAreaBoxCoordinates(list: List[Box]): Option[List[Box]] = {
    ...
    if (pointAndArea._2 == 0) {
      ...
      println(s"all points before sorting are ${pointAndArea._1}")
      ...
      println(s"all max area boxes are ${onlyMinAndMax}")
      Some(onlyMinAndMax)
    }
  }
  ...
  def main(args: Array[String]) = {
    ...
    println(s"connected boxes are ${output}")
    val connectedNonOverlappingSet = findUniqueSet(output)
    println(s"unique set are ${connectedNonOverlappingSet}")
    val maxAreaPointsOpt = findMaxAreaBoxCoordinates(connectedNonOverlappingSet)
    if (maxAreaPointsOpt.isEmpty) println(s"()")
    else {
